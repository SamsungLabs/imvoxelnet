...
class AtlasNeckV2(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels * 2, out_channels, 3, stride=(1, 1, 2), padding=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True)
        )
...
class AtlasNeckV3(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.model = nn.Sequential(
            ...
            nn.BatchNorm3d(in_channels * 2),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels * 2, out_channels, 3, stride=(2, 2, 2), padding=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True)
        )
