...
class Transformer3DHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_iou=dict(type='GIoU3DLoss', loss_weight=2.0),
                 train_cfg=dict(
                     assigner=dict(
                         ...
                         bbox_weight=5.,
                         iou_weight=2.,
                         iou_calculator=dict(type='BboxOverlaps3D', coordinate='depth'),
                         iou_mode='iou'
                     )),
    ...
    def _init_layers(self):
        ...
        self.reg_ffn = FFN(
            ...
            dropout=0.0,
            add_residual=False)
        self.fc_reg = Linear(self.embed_dims, 7)
    ...
    def forward_single(self, x, img_metas):
        ...
        all_bbox_preds = torch.cat((
            all_bbox_preds[..., :2],  # TODO: sigmoid ?
            torch.exp(all_bbox_preds[..., 2:6]),
            all_bbox_preds[..., 6:]
        ), dim=-1)
        return all_cls_scores, all_bbox_preds

    @staticmethod
    def bboxes_2d_to_3d(bboxes, img_meta):
        img_h, img_w, _ = img_meta['ori_shape']
        x_2d = bboxes[:, 0] * img_w
        y_2d = bboxes[:, 1] * img_h
    ...
    def loss(self,
        ...
        """
        # NOTE defaultly only the outputs from the last feature scale is used.x
    ...
    def loss_single(self,
        ...
        bbox_preds_list = [self.bboxes_2d_to_3d(
            bbox_preds[i], img_metas[i]) for i in range(num_imgs)]
        ...
        bbox_preds = torch.cat(bbox_preds_list, 0)  # my modification
    ...
    def _get_target_single(self,
        ...
        bbox_targets[pos_inds] = sampling_result.pos_gt_bboxes  # pos_gt_bboxes_targets
...
class HungarianAssigner3D(HungarianAssigner):
    ...
    def assign(self,
        ...
        overlaps = self.iou_calculator(
            bbox_pred, gt_bboxes, mode=self.iou_mode)  # , is_aligned=False)
